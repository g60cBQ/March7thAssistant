...
from module.notification import POWER_NOTIFS
    ...
    def prepare_instance(instance_type, instance_name):
        ...
        for i in range(10):
            if func():
                if auto.matched_text == "追踪":
                    time.sleep(2)
                    Base.notify_with_screenshot(POWER_NOTIFS.INSTANCE_INCOMPLETE, error="指定副本未解锁")
                    auto.press_key("esc")
                    auto.press_key("esc")
            ...
            time.sleep(1)
        if not Flag:
            Base.notify_with_screenshot(POWER_NOTIFS.INSTANCE_INCOMPLETE, error="未找到指定副本")
            return False
        # 验证传送是否成功
        if "饰品提取" in instance_type:
            if not auto.find_element(instance_name, "text", max_retries=120, include=True, crop=(591.0 / 1920, 98.0 / 1080, 594.0 / 1920, 393.0 / 1080)):
                Base.notify_with_screenshot(POWER_NOTIFS.INSTANCE_INCOMPLETE, error="传送可能失败")
                return False
        else:
        ...
        if "饰品提取" in instance_type:
            ...
            if auto.click_element("开始挑战", "text", max_retries=10, crop=(1558.0 / 1920, 939.0 / 1080, 216.0 / 1920, 70.0 / 1080)):
                ...
                if auto.find_element("./assets/images/purefiction/prepare_fight.png", "image", 50000, max_retries=60, crop=(0 / 1920, 0 / 1080, 300.0 / 1920, 300.0 / 1080)):
                    ...
                    Base.notify_with_screenshot(POWER_NOTIFS.INSTANCE_INCOMPLETE, error="无法开始挑战")
                    auto.press_key("esc")
                    time.sleep(2)
                    auto.press_key("esc")
