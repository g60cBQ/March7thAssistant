...
from module.notification import CHALLENGE_NOTIFS
    ...
    def collect_rewards(self):
        ...
        if self.max_level > 0:
            ...
            if auto.click_element("./assets/images/share/base/RedExclamationMark.png", "image", 0.9, max_retries=5, crop=(1775.0 / 1920, 902.0 / 1080, 116.0 / 1920, 110.0 / 1080)):
                ...
                Base.notify_with_screenshot(CHALLENGE_NOTIFS.LEVEL_CLEARED, name=self.name, level=self.max_level)

                auto.press_key("esc")
                time.sleep(1)
            else:
                log.error("领取星琼失败")
                Base.notify_with_screenshot(CHALLENGE_NOTIFS.LEVEL_CLEARED_WITH_ISSUE, name=self.name, level=self.max_level)
