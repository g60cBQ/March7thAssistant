...
from module.notification.types import *
from module.notification.template import *
...
def _migrate_legacy_notify_level():
    """
    迁移旧版 notify_level 配置到 NotifyLevel 枚举值。
    """
    try:
        default_value = NotifyLevel.NORMAL.value

        notify_level = cfg.get_value("notify_level", None)
        if not notify_level or notify_level == "all":
            cfg.set_value("notify_level", default_value)
            return

        valid_levels = {level.value for level in NotifyLevel}
        if notify_level not in valid_levels:
            cfg.set_value("notify_level", default_value)
    except Exception as e:
        log.error(f"迁移 notify_level 配置时出错: {e}")


def _build_notifier(notifier_name, params, logger):
    """
    ...
    elif notifier_name in _notifier_classes:
        return _notifier_classes[notifier_name](params, logger)
    else:
        # 默认情况下，如果没有找到匹配的类，则创建OnepushNotifier实例
        return OnepushNotifier(notifier_name, params, logger)


def get_notify_rules(policy_name: str = "default"):
    """从配置的策略列表中提取指定名称的策略规则。"""
    policies = cfg.get_value("notify_policies", [])
...
def set_notify_rules(policy_name: str = "default", rules: dict[str, str] = {}):
    """更新具有指定名称的通知策略规则配置。"""
    policies = cfg.get_value("notify_policies", [])
    ...
    for policy in policies:
        if isinstance(policy, dict) and policy.get("name") == policy_name:
            policy["rules"] = rules
            cfg.set_value("notify_policies", policies)
            return

    new_policy = {"name": policy_name, "rules": rules}
...
def reload_notifiers():
    ...
    for key, value in cfg.config.items():
        if key.startswith("notify_") and key.endswith("_enable") and value:
            ...
            params = {
                param_key[len("notify_" + notifier_name + "_") :]: param_value
            ...
            notifier = _build_notifier(notifier_name, params, log)
            notif.set_notifier(notifier_name, notifier)


_migrate_legacy_notify_level()

_title = cfg.get_value("notify_title", "三月七小助手|･ω･)")
_level = NotifyLevel(cfg.get_value("notify_level", NotifyLevel.NORMAL))
_rule = get_notify_rules()

notif = Notification(log, _title, _level, _rule)
